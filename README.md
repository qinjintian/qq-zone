<div align="center">
  ...
  <p>
    ...
    <a href="https://github.com/qinjintian/qq-zone/blob/main/LICENSE"><img src="https://img.shields.io/badge/License-个人学习-blue?style=flat-square" alt="License"></a>
...
<p align="center">
  <img src="docs/images/demo.png" alt="终端扫码登录后备份相册、群相册、说说或留言板" width="920">
</p>
...
QQ 空间里堆着很多人的照片、视频、说说和留言，群里也常有多年聚会和班级照片。手动保存又慢，相册还经常变成压缩图，说说和留言板更是只能在网页里翻。本工具用 **Go** 重写：相册支持 **原图 / 原视频 / 实况图** 并 **保留拍摄信息（EXIF）**；群相册同样下原图，按群分开放；说说会连同配图、视频、评论、点赞和浏览数一起拉下来；留言板会保存留言、回复、配图和主人寄语。说说和留言板都会生成一份 **可双击打开的本地时间线网页**。自己的空间、好友对你开放的空间，以及你所在群的相册都可以备份，手机 QQ **扫码登录** 即可。

## ✨ 核心特性

- 📸 **原图 + EXIF，不是压缩预览**：按相册原始分辨率下载，拍摄信息跟着走，备份下来才是自己的底片。
- 📂 **群相册也能下原图**：班级照、聚会相册先选群再勾相册，原图和视频按群分开放；名单不全时可以手输群号。
- 💬 **说说备份成本地网页**：正文、配图、视频、语音、转发、评论、点赞人和浏览次数一并保存；生成 `index.html`，双击就能按时间线翻看，不用对着 JSON 发愁。
- 💌 **留言板同样能备份**：留言、回复、配图和主人寄语一起保存；私密留言只留占位，不会把看不到的正文写进本地。
- 🎬 **能播就能下，地址失效自动换源**：相册视频、实况图一并收下；点下载失败时会改走播放地址再试，尽量把文件救回来。
- 📱 **扫码即用**：终端直接出二维码，手机 QQ 一扫就登录；账号会记住，换号也不用重新折腾。
- 👥 **自己的、好友的、群里的，勾了就下**：已有文件自动跳过，隔几个月再跑一遍只补新的。
...
- 按年份跳转，搜索正文 / 评论（留言板为回复）
- 说说可筛选有图或有视频；留言板可筛选有图或私密
- 点开配图，播放视频和语音，展开全部评论或回复
- 评论者、回复者和点赞人优先显示你通讯录里的好友备注
- 留言板会置顶主人寄语；私密留言显示占位文案，不泄露正文
...
4. **配一下任务**：并发填 `auto` 即可；建议打开增量下载（跳过已有文件）。相册还可按年/月整理、导出元数据。说说和留言板可以选是否额外保存原始接口 JSON，以及结束后是否打开查看页。
5. **勾选相册**：空格勾选，也可点「全选」；直接回车则备份全部。`Enter` 开始下载。群相册会先选群，再勾选相册。说说和留言板没有相册勾选，确认选项后即开始拉取。
6. **看结果**：下完会出任务报告。
...
**好友相册、说说或留言板为什么是空的？**  
对方没对你开放，或内容是私密的。可先用「查看对我开放的好友」确认哪些空间能进。访客看不见的说说会被跳过；留言板若无权访问，会在查看页顶部提示，不会覆盖你本地已有的备份。

**群列表里没有我想要的群，或提示无权访问？**  
前面扫码只登录了空间，列不出你加入的全部群。选「查看我加入的全部群」会再扫一次，专门授权看群名单，不是重新登录。知道群号也可以直接手输。若已经选对群仍提示无权访问，多半是已退群，或该相册未对你开放。

**群相册保存在哪？**  
每个群一个文件夹：`storage/qzone/<QQ号>/qun/<群号>/<相册名>/`。和个人相册一样支持增量、按年/月整理和失败重试。
...
**留言板备份下来怎么看？**  
备份完成后会生成 `storage/qzone/<QQ号>/liuyanban/index.html`。同样双击打开，可按年份浏览、搜索留言和回复、筛选有图或私密。主人寄语会显示在顶部。私密留言只保存占位说明，看不到的正文不会写进本地。
...
日常使用可以跳过。想改代码时：登录和空间协议在 `internal/qzone`，相册/群相册/说说/留言板备份在 `internal/app`，下载在 `internal/net`，终端菜单在 `internal/cli`。
...
├── docs/               # 项目文档与演示资料
├── internal/           # 私有业务逻辑
│   ├── app/            # 核心引擎（相册/群相册下载 + 说说/留言板备份）
│   │   └── viewer/     # 离线时间线页面（HTML/CSS/JS，说说与留言板共用）
│   ├── cli/            # 终端交互界面
│   ├── net/            # 网络请求封装
│   ├── pkg/            # 公共工具包 (Logger, Util)
│   └── qzone/          # QQ 空间协议实现
├── scripts/            # 跨平台编译脚本（支持 Windows 下构建全平台二进制程序）
├── storage/            # 数据存储（日志、相册、群相册、说说、留言板、任务记录、配置）
└── go.mod              # 依赖管理
```
