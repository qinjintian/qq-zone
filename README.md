<div align="center">
  <h1>QQ 空间备份（相册原图 / 原视频 / 说说 / 留言板）</h1>
  <p><strong>扫码登录，一键备份到本地。</strong> 相册下原图和原视频；说说和留言板连同配图一起保存，并生成可双击打开的网页。</p>
  <p>
...
<p align="center">
  <img src="docs/images/demo.png" alt="终端扫码登录后备份相册、说说或留言板" width="920">
</p>
...
QQ 空间里堆着很多人的照片、视频、说说和留言。手动保存又慢，相册还经常变成压缩图，说说和留言板更是只能在网页里翻。本工具用 **Go** 重写：相册支持 **原图 / 原视频 / 实况图** 并 **保留拍摄信息（EXIF）**；说说会连同配图、视频、评论、点赞和浏览数一起拉下来；留言板会保存留言、回复、配图和主人寄语。两者都会生成一份 **可双击打开的本地时间线网页**。自己的空间和好友对你开放的空间都可以备份，手机 QQ **扫码登录** 即可。
...
5. **勾选相册**：空格勾选，也可点「全选」；直接回车则备份全部。`Enter` 开始下载。说说和留言板没有相册勾选，确认选项后即开始拉取。
6. **看结果**：下完会出任务报告。
...
**文件保存在哪？**  
相册：`storage/qzone/<QQ号>/album/`。说说：`storage/qzone/<QQ号>/shuoshuo/`。留言板：`storage/qzone/<QQ号>/liuyanban/`（其中 `index.html` 是查看页）。任务记录在 `storage/tasks/`。

---

## 🛠️ 技术栈

日常使用可以跳过。想改代码时：登录和空间协议在 `internal/qzone`，相册/说说/留言板备份在 `internal/app`，下载在 `internal/net`，终端菜单在 `internal/cli`。
...
├── storage/            # 数据存储（日志、相册、说说、留言板、任务记录、配置）
└── go.mod              # 依赖管理
```
